<template>
  <div id="my-token" class="mb-14 my-token">
    <div v-if="dataList.length > 0 || loading" class="my-token-header">
      <h1 class="header" :loading="loading">My Tokens</h1>
      <p class="content-text">
        <!-- Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed interdum
        eros fringilla mattis luctus. Cras tristique metus et erat tincidunt
        laoreet. Nam at blandit ipsum. -->
      </p>
      <div class="search">
        <v-text-field
          v-model="search"
    ...
    <div v-if="dataList.length > 0 || loading" class="datatable">
      <v-data-table
        :headers="headers"
        :items="dataList"
        :search="search"
    ...
    <div v-if="dataList.length == 0 && !loading" class="no-token">
      <v-img src="@/assets/no-token.png" max-width="450" contain />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getAllTokens() {
      this.loading = true;
      const ether_address = await getEthAddress();
      getTokens({ ether_address, inComplete: false })
        .then(({ data }) => {
